        ...
        <div className="flex justify-between h-16 items-center gap-4">
          ...
          <div className="flex items-center gap-6 shrink-0">
            <Link to="/dashboard" className="group flex items-center gap-2.5">
              ...
              <div className="hidden lg:flex flex-col -space-y-1">
                ...
                <span className="text-[9px] text-muted-foreground font-medium uppercase tracking-[0.2em]">
                  Texas Community
                </span>
              </div>
            </Link>
                  ...
                  <div className="mb-4">
                    ...
                    {searchResults.length === 0 && !searchLoading ? (
                      ...
                      searchResults.map((res) => (
                        <div
                          key={res._id}
                          className="flex items-center justify-between p-2 hover:bg-muted rounded-xl transition-colors group"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full overflow-hidden bg-primary/10 border border-border flex items-center justify-center">
                              {res.profileImage &&
                              res.profileImage.trim() !== "" ? (
                                <img
                                  src={res.profileImage}
                                  className="w-full h-full object-cover"
                                  alt={`${res.firstName}'s profile`}
                                  onError={(e) => {
                                    // Self-heal: If the database URL is broken, swap to default image
                                    (e.currentTarget as HTMLImageElement).src =
                                      defaultProfile;
                                  }}
                                />
                              ) : (
                                /* Fallback to your custom default asset */
                                <img
                                  src={defaultProfile}
                  ...
                  <div>
                    ...
                    {eventResults.length === 0 && !searchLoading ? (
                      ...
                      eventResults.map((event) => (
                        <div
                          key={event._id}
                          className="flex items-center justify-between p-2 hover:bg-muted rounded-xl transition-colors"
                        >
                          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Popover onOpenChange={(open) => open && markAsRead()}>
              ...
              <PopoverContent
                className="w-80 p-0 shadow-2xl border-border/50 rounded-xl"
                align="end"
              >
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n, i) => (
                      <div
                        key={i}
                        className={cn(
                          "p-4 border-b flex flex-col gap-2 transition-colors",
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className="flex items-center gap-2 p-1 pl-2 pr-1 rounded-full border border-transparent hover:border-border hover:bg-muted transition-all outline-none group">
                  <p className="hidden sm:block text-xs font-bold leading-none">
                    {user?.firstName}
                  </p>
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20 overflow-hidden shrink-0 group-hover:scale-105 transition-transform">
                    {user?.profileImage && user.profileImage.trim() !== "" ? (
                      <img
                        ...
                        onError={(e) => {
                          // If the specific user's image fails to load, show the community default
                          (e.currentTarget as HTMLImageElement).src =
                            defaultProfile;
                        }}
                      />
                    ) : (
                      /* Replace initials with the default PNG asset */
                      <img
                        src={defaultProfile}
                        className="w-full h-full object-cover"
                        alt="Default Profile"
              ...
              <PopoverContent
                className="w-64 p-3 rounded-2xl shadow-2xl border-border/60"
                align="end"
              >
                  ...
                  <div className="w-12 h-12 rounded-full bg-muted border-2 border-background shadow-sm overflow-hidden shrink-0">
                    {user?.profileImage && user.profileImage.trim() !== "" ? (
                      <img
                        ...
                        onError={(e) => {
                          // Fallback for broken database URLs
                          (e.currentTarget as HTMLImageElement).src =
                            defaultProfile;
                        }}
                      />
                    ) : (
                      /* Replace the single initial div with your branded default image */
                      <img
                        src={defaultProfile}
                        className="w-full h-full object-cover"
                        alt="Default Profile"
                ...
                <div className="space-y-1 pt-1">
                  <Link to="/profile">
                    <Button
                      variant="ghost"
                      className="w-full justify-start h-11 text-sm gap-3 rounded-xl px-2.5"
                    >
